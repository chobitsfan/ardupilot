...
void NavEKF2_core::ResetHeight(void)
{
    ...
    } else if (inFlight && extNavVelMeasTime_ms) {
...
void NavEKF2_core::FuseVelPosNED()
{
    ...
    if (fuseVelData || fusePosData || fuseHgtData) {
        ...
        if (fuseVelData) {
            ...
            if ((extNavVelMeasTime_ms == 0) && (frontend->_fusionModeGPS > 0 || PV_AidingMode != AID_ABSOLUTE || frontend->inhibitGpsVertVelUse)) {
                imax = 1;
            }
        ...
        if (fuseVelData && velHealth) {
            fuseData[0] = true;
            fuseData[1] = true;
            if (useGpsVertVel || extNavVelMeasTime_ms) {
                fuseData[2] = true;
            }
            tiltErrVec.zero();
        }
...
void NavEKF2_core::selectHeightForFusion()
{
    ...
    hgtRetryTime_ms = ((useGpsVertVel || extNavVelMeasTime_ms) && !velTimeout) ? frontend->hgtRetryTimeMode0_ms : frontend->hgtRetryTimeMode12_ms;
