...
void GCS_MAVLINK::handle_att_pos_mocap(const mavlink_message_t &msg)
{
    ...
    visual_odom->handle_pose_estimate(m.time_usec, timestamp_ms, m.x, m.y, m.z, m.q, 0, 0, 0, m.latency);
}
...
void GCS_MAVLINK::handle_vision_speed_estimate(const mavlink_message_t &msg)
{
    ...
    visual_odom->handle_vision_speed_estimate(m.usec, timestamp_ms, vel, m.reset_counter, m.latency);
}
