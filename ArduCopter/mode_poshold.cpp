...
bool ModePosHold::init(bool ignore_checks)
{
    ...
    pos_control->set_max_speed_accel_xy(500.0f/*POSCONTROL_SPEED*/, 100.0f/*POSCONTROL_ACCEL_XY*/);
...
float ModePosHold::get_fence_adjusted_climbrate(float target_rate) {
    ...
    if (fence && fence->enabled()) {
        ...
        if (_ahrs.get_relative_position_D_origin(veh_alt)) {
            if (target_rate > 0) {
                // _fence.get_safe_alt_max() is UP, veh_alt is DOWN:
                alt_diff = fence->get_alt_max() + veh_alt;
                ...
                alt_diff = fence->get_alt_min() + veh_alt;
                if (alt_diff >= 0.0f) {
                    return 0;
                }
                alt_diff = -alt_diff;
...
void ModePosHold::run()
{
    ...
    get_pilot_desired_lean_angles(target_roll, target_pitch, g.poshold_angle_max, attitude_control->get_althold_lean_angle_max_cd());
